...
(() => {
  const qAllSections = () => Array.from(document.querySelectorAll('[data-events]'));

  // Icons (inline so cards are self-contained)
  ...
  const fmtDate = (dateStr) => {
    try {
      ...
      return dt.toLocaleDateString('es', { month:'long', day:'numeric', year:'numeric' });
  ...
  function makeMonthAccordionItem(key, events, isFirst = false) {
    ...
    if (isFirst) panel.classList.add('open'); // only first open
  ...
  // Month accordion (used on full page)
  function renderMonthAccordion(section, list) {
    // Hide placeholder grid (keep it in DOM in case section switches modes later)
    const placeholder = section.querySelector('.events__grid, #events-grid');
    if (placeholder) placeholder.hidden = true;

    // Find or create the months host
    let host = section.querySelector('[data-months] .accordion-faq__wrapper');
    if (!host) {
      // fallback: create host if missing
      const shell = document.createElement('section');
      shell.className = 'section--accordion-faq';
      shell.setAttribute('data-months','');
      const wrap = document.createElement('div');
      wrap.className = 'accordion-faq__wrapper';
    ...
    // Group events by month and render items
    ...
    [...buckets.keys()].sort().forEach(k => {
      frag.appendChild(makeMonthAccordionItem(k, buckets.get(k)));
    });
    host.appendChild(frag);
  }

  function renderInto(section, allEvents) {
    const isMonthMode = section.hasAttribute('data-month-accordion');
    ...
    if (!list.length) {
      ...
      const host = section.querySelector('[data-months] .accordion-faq__wrapper');
    ...
    if (isMonthMode) {
      renderMonthAccordion(section, list);
    } else {
      renderSimpleGrid(grid, list);
    }
  }

  // Public API – called by your Supabase bridges
  window.Rail = Object.assign(window.Rail || {}, {
    setEvents(allEvents = []) { qAllSections().forEach(sec => renderInto(sec, allEvents)); },
    init() {}
  });
})();
