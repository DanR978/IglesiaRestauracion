...
// "Tesorería" — church-wide treasury + per-ministry budgets.
//   Church level: total inflow (income) and allocation (expenses), balance,
//   payables, recurring stipends, monthly reports.
//   Ministry level: each ministry has its own annual budget; spend rolls up.
// Full control for admins/treasurer (RLS). Ministry leaders get read-only
// budget/spend for their own ministry.
...
function minOptions(sel) {
  return `<option value="">— General —</option>` +
    ministries.map(m => `<option value="${m.id}"${m.id === sel ? ' selected' : ''}>${esc(m.name)}</option>`).join('');
}
...
let editing = {};           // key -> row id being edited
const cache = {};           // key -> last fetched rows (for edit prefill)

const yStart = () => `${year}-01-01`;
const yEnd   = () => `${year}-12-31`;

export async function loadTreasury() {
  const sel = document.getElementById('trezYear');
  if (sel && !sel.options.length) {
    const y0 = new Date().getFullYear();
    sel.innerHTML = [y0 + 1, y0, y0 - 1, y0 - 2, y0 - 3].map(y => `<option value="${y}">${y}</option>`).join('');
    sel.value = String(year);
    sel.addEventListener('change', () => { year = Number(sel.value); editing = {}; render(); });
  }
...
function syncNav() {
  document.querySelectorAll('#trezSubnav .trez-subnav__btn')
...
async function renderResumen(root) {
  const [inc, exp, bud, pay] = await Promise.all([
    sb.from('fin_income').select('amount').gte('occurred_on', yStart()).lte('occurred_on', yEnd()),
    sb.from('fin_expenses').select('amount,ministry_id').gte('occurred_on', yStart()).lte('occurred_on', yEnd()),
    sb.from('fin_budgets').select('ministry_id,amount').eq('period', String(year)),
  ...
  const spentByMin = {}; (exp.data || []).forEach(e => spentByMin[e.ministry_id] = (spentByMin[e.ministry_id] || 0) + Number(e.amount || 0));
  const budByMin = {};   (bud.data || []).forEach(b => budByMin[b.ministry_id] = Number(b.amount || 0));
  const rows = ministries.map(m => ({ name: m.name, budget: budByMin[m.id] || 0, spent: spentByMin[m.id] || 0 }))
    .filter(r => r.budget || r.spent);

  const tiles = [
    { icon: 'fa-arrow-down', cls: 'in',  num: fmt(income),   label: `Ingresos ${year}` },
    { icon: 'fa-arrow-up',   cls: 'out', num: fmt(expenses), label: `Gastos ${year}` },
  ...
  root.innerHTML = `
    <p class="trez-lead">Tesorería de toda la iglesia — entrada total de dinero y su asignación por ministerio.</p>
    <div class="trez-tiles">
      ${tiles.map(t => `<div class="trez-tile trez-tile--${t.cls}">
        <span class="trez-tile__icon"><i class="fas ${t.icon}"></i></span>
        <span class="trez-tile__num">${t.num}</span>
        <span class="trez-tile__label">${t.label}</span></div>`).join('')}
    </div>
    <div class="trez-card">
      <h3 class="trez-card__title"><i class="fas fa-scale-balanced"></i> Asignación por ministerio · ${year}</h3>
      ${rows.length ? `<div class="trez-budtable">${rows.map(r => {
        ...
        : `<div class="empty-state"><i class="fas fa-scale-balanced"></i><p>Aún no hay presupuestos ni gastos para ${year}.</p></div>`}
    </div>`;
}

/* ── Ingresos (money gathered ledger) ──────────────────────────────────────── */
async function renderIncome(root) {
  const { data, error } = await sb.from('fin_income').select('*')
    .gte('occurred_on', yStart()).lte('occurred_on', yEnd()).order('occurred_on', { ascending: false });
  ...
  root.innerHTML = `
    ...
    `)}
    <div class="trez-total">Total recaudado ${year}: <strong class="pos">${fmt(sum(data))}</strong></div>
    ${table(['Fecha', 'Fuente', 'Fondo', 'Monto', ''], (data || []).map(r => `
      <tr>
        ...
        ${rowActions(r.id)}
      </tr>`).join(''), 'No hay ingresos registrados.')}`;
  ...
  bindForm(root, 'income', 'fin_income', () => {
    ...
    return { payload: {
      ...
      amount, note: document.getElementById('inNote').value.trim() || null,
...
async function renderExpenses(root) {
  ...
  root.innerHTML = `
    ${addForm('expenses', ed, `
      ...
      ${field('exMin', 'Ministerio', `<select id="exMin">${minOptions(ed?.ministry_id)}</select>`)}
    ...
    `)}
    <div class="trez-total">Total gastado ${year}: <strong class="neg">${fmt(sum(data))}</strong></div>
    ${table(['Fecha', 'Ministerio', 'Pagado a', 'Monto', 'Estado', ''], (data || []).map(r => `
      <tr>
        <td>${fmtDate(r.occurred_on)}</td>
        <td>${esc(minName(r.ministry_id))}${r.category ? `<span class="trez-sub">${esc(r.category)}</span>` : ''}</td>
        <td>${esc(r.payee || '—')}${r.note ? `<span class="trez-sub">${esc(r.note)}</span>` : ''}</td>
        <td class="num neg">${fmt(r.amount)}</td>
        <td>${statusPill(r.status)}</td>
        ${rowActions(r.id)}
      </tr>`).join(''), 'No hay gastos registrados.')}`;
  ...
  bindForm(root, 'expenses', 'fin_expenses', () => {
    ...
    return { payload: {
      ...
      ministry_id: document.getElementById('exMin').value || null,
...
async function renderRecurring(root) {
  ...
  root.innerHTML = `
    <p class="trez-lead">Pagos que se repiten — estipendios, renta, suscripciones. Usa “Registrar” para llevarlo al libro de gastos de este mes.</p>
    ${addForm('recurring', ed, `
      ${field('rcPayee', 'Pagado a', `<input type="text" id="rcPayee" value="${esc(ed?.payee || '')}" placeholder="Pastor — estipendio">`)}
      ${field('rcMin', 'Ministerio', `<select id="rcMin">${minOptions(ed?.ministry_id)}</select>`)}
    ...
    `)}
    ${table(['Pagado a', 'Ministerio', 'Monto', 'Frecuencia', 'Activo', ''], (data || []).map(r => `
      <tr class="${r.active ? '' : 'is-paid'}">
        ...
        <td>${r.active ? '<span class="trez-pill trez-pill--paid">Sí</span>' : '<span class="trez-pill trez-pill--pending">No</span>'}</td>
        <td class="row-act">
          <button class="adm-icon-btn" data-post="${r.id}" title="Registrar este mes en gastos"><i class="fas fa-file-circle-plus"></i></button>
          <button class="adm-icon-btn" data-edit="${r.id}" title="Editar"><i class="fas fa-pen"></i></button>
          <button class="adm-icon-btn" data-del="${r.id}" title="Eliminar"><i class="fas fa-trash"></i></button>
        </td>
  ...
  bindForm(root, 'recurring', 'fin_recurring', () => {
    ...
    if (!(amount >= 0)) return 'Escribe un monto válido.';
    return { payload: {
      payee, ministry_id: document.getElementById('rcMin').value || null,
  ...
  // "Registrar este mes" → spawn an expense from the recurring template
  root.querySelectorAll('[data-post]').forEach(btn => btn.addEventListener('click', async () => {
    const r = (cache.recurring || []).find(x => x.id === btn.dataset.post);
    if (!r) return;
    const t = new Date();
    ...
    const { error: e } = await sb.from('fin_expenses').insert({
      occurred_on: occ, ministry_id: r.ministry_id, payee: r.payee,
...
/* ── Presupuestos (per ministry, annual) ───────────────────────────────────── */
async function renderBudgets(root) {
  const { data, error } = await sb.from('fin_budgets').select('*').eq('period', String(year));
  if (error) { root.innerHTML = errBox(error); return; }
  const byMin = Object.fromEntries((data || []).map(b => [b.ministry_id, b]));

  root.innerHTML = `
    <div class="trez-card">
      <h3 class="trez-card__title"><i class="fas fa-scale-balanced"></i> Presupuesto por ministerio · ${year}</h3>
      <p class="trez-hint">Cada ministerio tiene su propio presupuesto anual. Se guarda al instante.</p>
      <div class="trez-budset">
        ${ministries.length ? ministries.map(m => `
          <div class="trez-budset__row">
            <span class="trez-budset__name">${esc(m.name)}</span>
            <div class="trez-budset__input"><span>$</span>
              <input type="number" min="0" step="0.01" data-bud="${m.id}" value="${byMin[m.id]?.amount ?? ''}" placeholder="0.00"></div>
            <button class="btn btn--ghost btn--sm" data-budsave="${m.id}">Guardar</button>
          </div>`).join('') : '<div class="empty-state"><i class="fas fa-church"></i><p>Crea ministerios primero.</p></div>'}
      </div>
    </div>`;

  root.querySelectorAll('[data-budsave]').forEach(btn => btn.addEventListener('click', async () => {
    const id = btn.dataset.budsave;
    const amount = parseFloat(root.querySelector(`[data-bud="${id}"]`).value);
    if (!(amount >= 0)) { toast('Monto inválido', 'error'); return; }
    btn.disabled = true;
    const { error: e } = await sb.from('fin_budgets')
      .upsert({ ministry_id: id, period: String(year), amount, updated_at: new Date().toISOString() }, { onConflict: 'ministry_id,period' });
    btn.disabled = false;
    if (e) { toast(e.message, 'error'); return; }
...
async function renderPayables(root) {
  ...
  root.innerHTML = `
    ${addForm('payables', ed, `
      ...
      ${field('pyMin', 'Ministerio', `<select id="pyMin">${minOptions(ed?.ministry_id)}</select>`)}
    ...
    ${table(['Se le debe a', 'Monto', 'Vence', 'Ministerio', 'Estado', ''], (data || []).map(r => `
      <tr class="${r.status === 'paid' ? 'is-paid' : ''}">
        ...
        <td>${esc(minName(r.ministry_id))}</td>
        <td>${r.status === 'paid' ? statusPill('paid') : '<span class="trez-pill trez-pill--owe">Abierto</span>'}</td>
        <td class="row-act">
          ${r.status === 'open' ? `<button class="adm-icon-btn" data-paid="${r.id}" title="Marcar pagado"><i class="fas fa-check"></i></button>` : ''}
          <button class="adm-icon-btn" data-edit="${r.id}" title="Editar"><i class="fas fa-pen"></i></button>
          <button class="adm-icon-btn" data-del="${r.id}" title="Eliminar"><i class="fas fa-trash"></i></button>
        </td>
  ...
  bindForm(root, 'payables', 'fin_payables', () => {
    ...
    return { payload: {
      ...
      note: document.getElementById('pyNote').value.trim() || null,
      created_by: currentUser?.id || null,
    } };
  });
  root.querySelectorAll('[data-paid]').forEach(btn => btn.addEventListener('click', async () => {
    const { error: e } = await sb.from('fin_payables').update({ status: 'paid', paid_at: new Date().toISOString() }).eq('id', btn.dataset.paid);
    if (e) { toast(e.message, 'error'); return; }
    toast('Marcado como pagado', 'success'); render();
  }));
}

/* ── Reportes (monthly, past months) ───────────────────────────────────────── */
async function renderReports(root) {
  const [inc, exp] = await Promise.all([
    sb.from('fin_income').select('occurred_on,amount').gte('occurred_on', yStart()).lte('occurred_on', yEnd()),
    sb.from('fin_expenses').select('occurred_on,amount').gte('occurred_on', yStart()).lte('occurred_on', yEnd()),
  ]);
  if (inc.error || exp.error) { root.innerHTML = errBox(inc.error || exp.error); return; }

  const months = Array.from({ length: 12 }, () => ({ in: 0, out: 0 }));
  (inc.data || []).forEach(r => { months[Number(r.occurred_on.slice(5,7)) - 1].in  += Number(r.amount || 0); });
  (exp.data || []).forEach(r => { months[Number(r.occurred_on.slice(5,7)) - 1].out += Number(r.amount || 0); });

  let ytdIn = 0, ytdOut = 0;
  const rows = months.map((m, i) => {
    ytdIn += m.in; ytdOut += m.out;
    return { name: MONTHS[i], in: m.in, out: m.out, bal: m.in - m.out, ytd: ytdIn - ytdOut, has: m.in || m.out };
  });
  const totIn = sum(inc.data), totOut = sum(exp.data);

  root.innerHTML = `
    <p class="trez-lead">Reporte mensual de ${year}. Cambia el año arriba para ver meses anteriores.</p>
    <div class="trez-tablewrap"><table class="trez-table trez-report">
      ...
      <tbody>
        ${rows.map(r => `<tr class="${r.has ? '' : 'is-muted'}">
          <td>${r.name}</td>
          <td class="num pos">${r.in ? fmt(r.in) : '—'}</td>
          <td class="num neg">${r.out ? fmt(r.out) : '—'}</td>
          <td class="num ${r.bal < 0 ? 'neg' : 'pos'}">${(r.in||r.out) ? fmt(r.bal) : '—'}</td>
          <td class="num ${r.ytd < 0 ? 'neg' : ''}">${fmt(r.ytd)}</td>
        </tr>`).join('')}
      </tbody>
...
async function renderNotes(root) {
  ...
  root.innerHTML = `
    ${addForm('notes', null, `
      ${field('ntBody', 'Nota o recordatorio', `<input type="text" id="ntBody" placeholder="Ej. Pagar al músico el viernes">`, true)}
      ${field('ntMin', 'Ministerio', `<select id="ntMin">${minOptions('')}</select>`)}
      ${field('ntPin', 'Fijar', `<label class="trez-check"><input type="checkbox" id="ntPin"> Mantener arriba</label>`)}
    `)}
    <div class="trez-notes">
      ${(data || []).length ? (data || []).map(n => `
        <div class="trez-note${n.pinned ? ' pinned' : ''}">
          ...
          <button class="adm-icon-btn trez-note__del" data-del="${n.id}" title="Eliminar"><i class="fas fa-trash"></i></button>
...
function rowActions(id) {
  return `<td class="row-act">
    <button class="adm-icon-btn" data-edit="${id}" title="Editar"><i class="fas fa-pen"></i></button>
    <button class="adm-icon-btn" data-del="${id}" title="Eliminar"><i class="fas fa-trash"></i></button></td>`;
}
...
function statusPill(s) {
  return s === 'paid' ? '<span class="trez-pill trez-pill--paid">Pagado</span>'
                      : '<span class="trez-pill trez-pill--pending">Pendiente</span>';
}
// One binder: handles add + edit (insert vs update) + row edit/delete/cancel.
function bindForm(root, key, tableName, build) {
  ...
  if (form) form.addEventListener('submit', async (e) => {
    ...
    btn.disabled = true;
    const q = editing[key]
      ? sb.from(tableName).update(res.payload).eq('id', editing[key])
      : sb.from(tableName).insert(res.payload);
    const { error } = await q;
    btn.disabled = false;
  ...
  root.querySelectorAll('[data-edit]').forEach(btn => btn.addEventListener('click', () => { editing[key] = btn.dataset.edit; render(); window.scrollTo(0, 0); }));
  root.querySelectorAll('[data-del]').forEach(btn => btn.addEventListener('click', async () => {
    const ok = await confirm('Eliminar', '¿Eliminar este registro? No se puede deshacer.');
    if (!ok) return;
